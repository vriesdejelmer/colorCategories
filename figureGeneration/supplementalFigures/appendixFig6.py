...
frequency_matrix = np.load('/Users/vriesdejelmer/Dropbox/pythonProjects/categoricalAnalysis/data/ImagenetPixData/rgb_frequency.npy')
...
hue_x = np.cos(hue_list*2*np.pi)
hue_y = np.sin(hue_list*2*np.pi)
...
frequency_array = np.zeros((7, steps), dtype=int)
...
stim_gen = HueStimulusGenerator(hue_convertor=HSVColor(brightness=1.0, saturation=1.0))
image_grid = stim_gen.getHueArray()

plt.imshow(image_grid, interpolation='nearest', aspect='auto')
plt.xlabel('Hue (%)', fontsize=18)
plt.yticks([])

plt.savefig('../../data/figures/appendix/' + 'appendix_figure6.png')